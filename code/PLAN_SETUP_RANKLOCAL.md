...
### 9-8. 프로덕션 스팟 검증 (2_iSMR_ECT_res1, np=4)

`CUPID_NP=4 scripts/run.sh` 실행 후 직전 동일 케이스 np=4 로그
(`run_20260821_1209_np4.log`)와 스텝 테이블 대조:
**비교 가능한 148 스텝 전 구간에서 TIME/DP_MAX/TOTAL_MASS/DT 완전 일치**
(신규 런은 같은 시간 예산에서 252 스텝까지 진행). 팬아웃·언팩 경로가 실제
MPI·METIS 분할에서도 무손상임을 확인.

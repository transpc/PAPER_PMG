...
| `tpfh2o` | 있음 | 증기표 |
| `somaFlow.in` | 있음 (이력 §7) | T/H 입력. 2026-08-09 사용자 제공(구입력 d145c81, 15 MPa) → 2026-08-11 상류 교체(**ECT1 신입력** 29ee238, 101.3 kPa 실증 — 호환성 검증 LOG C016). 골든·ref 는 입력 버전에 결박 (LOOP §2 입력 이원화) |
...
- `run_tests.sh`: 합성 유닛 테스트 → 골든 재생(세트별) 순으로 실행, 요약 표 출력. 모든 실행은 `in_contain.sh` 경유.
- **사이클 단위 운용 판정은 [LOOP.md](LOOP.md) §2 를 따른다** — 1차 지표는 its + 독립 계산 residual(`‖b−Au‖/‖b‖`), 위 표의 bitwise/해 비교는 마일스톤(L3) 검증용.
- **골든은 입력 버전에 결박** (2026-08-12, LOG C016/C017): 세트별 디렉토리(`golden/<세트>/meta.md` 에 입력 blob·복원 절차 명시)로 관리 — 현행 2세트 병행: `iSMR436k_np1`(구입력 s1/10/30) + `ECT1_436k_np1`(신입력 s1/10/30/150). 충실도 임계(fid_gate)는 세트별 인자 (예조건자 시드 문맥 의존의 발현 크기가 케이스마다 다름 — meta.md 근거 기록).
...
- 이행 안전망: 과도기에 "파일 모드/통신 모드" 스위치(`isetup_comm`, mg.in `&MG_MPI`, 기본 0=파일)를 두고 두 경로의 결과 일치(bitwise)를 확인한 뒤 파일 경로 삭제
- **bitwise 함정과 대응 (C011-1 실험 확정)**: 현 파일 경로는 REAL(8)을 list-directed ASCII 로 왕복시켜 **~67%가 라운딩**됨 (16자리 출력 vs 보존에 17자리 필요). 통신 모드가 정확값을 보내면 파일 모드와 원리적으로 bitwise 불일치 → 과도기 통신 모드는 REAL(8) 페이로드에 **내부 WRITE/READ 문자열 왕복 shim**(파일 라운딩과 bitwise 동일함을 실험 검증)을 적용해 전송 수단 교체와 수치 변화를 분리. shim 제거(정확값 전달)는 별도 사이클에서 §4-4 셋째 행(의도적 개선) 절차로 — its ±0 + 골든 재베이스라인
- 증분 로드맵 (LOG C011-1): ① `PMG_infor` 통신화(정수 메타 — O(np²) 스킵 소거) ② `part###.out`(finest) 2-phase Scatterv ③ `part_MG###.out`(레벨 루프, `A_GC` 는 유일한 중복 데이터라 Bcast) ④ shim 제거 ⑤ 파일 경로·MG_tmp 삭제
- 효과: `MG_tmp` 디렉토리 자체가 불필요해짐 — 대규모 np 실행의 구조적 병목 제거 (rank0 의 np개 fd 동시 점유·O(np²) 메타 스킵 포함). **셋업 단계 통신 구조 개선으로 논문 서사(통신 저감)와도 연결**
...
> (해소됨) `rv_parameters.in` 확보 — **rv_model 은 고려하지 않기로 결정 (2026-08-10)**. `rv_model=0` 상태를 기준 구성으로 확정하고, 골든 덤프 채취(C009)도 이 구성의 스모크 구간(step 1~38, 발산 전)에서 수행한다. step 38 물리 발산(추정: rv 모델 부재)은 **구입력(d145c81) 한정** — ECT1 신입력은 rv_model=0 이 기본이며 t=2.23s 까지 발산 없음 (LOG C016/C017).
> (해소됨) `somaFlow.in` — 2026-08-09 사용자 제공(구입력). 당시 소스가 모르던 신형 옵션 4개는 입력에서 주석 처리했음 (LOG C005-r3). **2026-08-11 상류 교체(ECT1) 이후 무효**: 새 입력의 `vfporous`/`i_droplet`/`i_fs_temp_intpol` 은 현 소스 `misc_option` 네임리스트에 선언되어 있고 `HS_coupling` 은 새 입력에서 제거됨 — 입력 수술 불필요 (LOG C016 호환성 검증).
...
Phase 4  ☑ 베이스라인 태그(baseline-phase4) + 성능 기준표(perf_log.md)      [LOG C015]
         ☑ G1(합성 기준): np=900 재현→원인 확정→수정→완주 green             [LOG C010-1~4]
           (잔여: 프로덕션 케이스 np=900 실증 — 클러스터 확보 시)
         ☑ 상류 입력 교체(ECT1) 대응 — 호환성 검증 + 골든 이원화             [LOG C016·C017]
         □ G2: MG_tmp 파일 왕복 → MPI 통신 대체 (과도기 이중 모드로 bitwise 검증)
         □ G3: 간소화·가독성 리팩터링 — 1차 완료(C012-1), 잔여: blaslapack IMPLICIT NONE 등

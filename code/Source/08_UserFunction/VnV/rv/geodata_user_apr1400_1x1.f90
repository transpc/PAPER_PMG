!
      SUBROUTINE GeoData_user_apr1400_1x1(nn)
!
...
!     NOT 1:1 mapping      
      USE Zrv_subchan , ONLY: nz_nk,dz_nk,nz_th0,hz_nk
      USE Zrv_subchan , ONLY: n_sg,h_sg,sg_loc
...
!     OPR1000
      IF(vv_prob.eq.'opr1000_mc_rv'          .or. &
         ...
         vv_prob.eq.'OPR1000_fullvessel_1x1'       )then
         nrf=64
         npinx=16
         top_rfl=2
      ENDIF   
!     APR1400
      IF(vv_prob.eq.'apr1400core_rv')then
         nrf=72
         npinx=16
         top_rfl=0
      ENDIF   
...
!
      ...
      nasm=i0/z0
      nchn=int(dsqrt(dble(i0/z0/nasm)))
      
      !total table
      ALLOCATE(asm0_to_cell(nasm*nchn,z0))
      asm0_to_cell=0
      !invrt table
      ALLOCATE(asmi_to_cell(nasm*nchn))
      ALLOCATE(asmz_to_cell(nasm*nchn))
      asmi_to_cell=0
      asmz_to_cell=0
...
!
      IF(l_spacer_grid)then
         ...
         IF(np.gt.1) then
            ...
            CALL broadcast_r1(h_sg)
         ENDIF
         IF(myrank.ne.0) then
            ALLOCATE(sg_loc(n_sg))
            sg_loc=0.0d0
         ENDIF
         CALL broadcast_r(sg_loc,n_sg)
         IF(myrank.eq.0) then
...
!     APR1400
      ELSEIF(vv_prob.eq.'apr1400core_rv'     .or. &
             vv_prob.eq.'apr1400_mc_rv' .or. &
             vv_prob.eq.'apr1400_rv'               )then
         ny=34
         ALLOCATE(ja_f(ny),ia_r(ny))
         ja_f=(/14,14, 22,22, 26,26, 30,30, 30,30, &   !5
                34,34, 34,34, 34,34, 34,34, 34,34, 34,34, 34,34, &   !10
                30,30, 30,30, 26,26, 22,22, 14,14   /) !15
         ia_r=(/42,12, 10, 8,  8, 8,  6, 4,  6, 8, &   !5
                 6, 4,  4, 4,  4, 4,  4, 4,  4, 4,  4, 4,  4, 4, &   !10
                 6, 8,  6, 4,  6, 8,  8, 8, 10,12   /) !15
      ENDIF
!
...
!      
      ...
      RETURN
      END SUBROUTINE GeoData_user_apr1400_1x1

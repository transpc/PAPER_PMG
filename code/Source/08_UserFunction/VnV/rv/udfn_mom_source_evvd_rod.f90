!
      SUBROUTINE udfn_mom_source_evvd_rod
...
!.....Local variables
      ...
      REAL(8) :: ka,g_i,g_k,g_avg,rho_avg
      ...
      REAL(8) :: beta_tmp
!.....Local arrays
      REAL(8),DIMENSION(ncell_fp) :: g_ik,rho_avgik,arij_g,arij_l,arij_lvd      !PSH
...
!
      IF(l_2p_multiplier_evvd) THEN
            DO i=1,ncell_fluid
               ...
               Rel=MAX(1.0d0,cell%rhol(i)*velo*hd/cell%lviscosl(i))
               x_x=cell%quals(i)
...
!.....> check that all those cell%..(kk) are communicated
      DO i=1,ncell_fp
         ...
         arij_g(i)  =      cell%alphag(i) *cell%rhog(i)     !PSH
...
if(1)then
      IF(ndim.eq.2) THEN
         ...
         IF(chn_type(ii).ne.0 .and. ABS(xn3).le.0.5d0) THEN
            ...
            void_drift_g_eng_nf(i0)=vt*void_drift_g_eng_non(i)      !PSH
...
!
         ...
         IF(chn_type(ii).ne.0 .and. ABS(xn3).le.0.5d0) THEN
            ...
            void_drift_g_eng_nf(i)=vt*void_drift_g_eng_non(k)   !PSH
...
!
      DO i=1,ncell_fluid
         IF(chn_type(i).eq.0)then
            tm_mas_l(i)=0.0d0
            tm_mas_g(i)=0.0d0
            tm_eng_l(i)=0.0d0
            tm_eng_g(i)=0.0d0
            vd_mas_l(i)=0.0d0   !PSH
            vd_mas_g(i)=0.0d0
            vd_eng_l(i)=0.0d0
            vd_eng_g(i)=0.0d0
            tm_mom_l=0.0d0
         ...
         ELSE
            ...
            !tm_mom_l=turb_mixing_l_mom(i,ndim)*TPM(i)/cell_area(i)     !PSH
            !tm_mom_g=turb_mixing_g_mom(i,ndim)*TPM(i)/cell_area(i)
            !vd_mom_l=void_drift_l_mom(i,ndim) *TPM(i)/cell_area(i)
            !vd_mom_g=void_drift_g_mom(i,ndim) *TPM(i)/cell_area(i)
            vd_mas_l(i)=0.0d0
            vd_mas_g(i)=0.0d0
            vd_eng_l(i)=0.0d0
            vd_eng_g(i)=0.0d0
            ...
            ENDDO
         ENDIF
         !src_liq(i,ndim)=src_liq(i,ndim)+(tm_mom_l+vd_mom_l)
         !src_gas(i,ndim)=src_gas(i,ndim)+(tm_mom_g+vd_mom_g)
      ENDDO
!
      END SUBROUTINE udfn_mom_source_evvd_rod
!
      SUBROUTINE udfn_mom_source_evvd_rod_old
...
!
      DO i=1,nf_non
         turb_mixing_l_mas_non(i)=0.0d0
         turb_mixing_g_mas_non(i)=0.0d0
         turb_mixing_l_eng_non(i)=0.0d0
         turb_mixing_g_eng_non(i)=0.0d0
         void_drift_l_mas_non(i)=0.0d0
         void_drift_g_mas_non(i)=0.0d0
         void_drift_l_eng_non(i)=0.0d0
         void_drift_g_eng_non(i)=0.0d0
         DO ix=1,ndim
            turb_mixing_l_mom_non(i,ix)=0.0d0
            turb_mixing_g_mom_non(i,ix)=0.0d0
            void_drift_l_mom_non(i,ix)=0.0d0
            void_drift_g_mom_non(i,ix)=0.0d0
         ENDDO
      ENDDO
...
!
      ...
      DO i=1,ncell_fluid
         ...
         ELSE
            ...
            vd_eng_l(i)=vd_eng_l(i)*TPM(i)/cell_area(i)
            vd_eng_g(i)=vd_eng_g(i)*TPM(i)/cell_area(i)
            tm_mom_l=turb_mixing_l_mom(i,ndim)*TPM(i)/cell_area(i)
            tm_mom_g=turb_mixing_g_mom(i,ndim)*TPM(i)/cell_area(i)
            vd_mom_l=void_drift_l_mom(i,ndim) *TPM(i)/cell_area(i)
            vd_mom_g=void_drift_g_mom(i,ndim) *TPM(i)/cell_area(i)
         ENDIF
...
!
      RETURN
      END SUBROUTINE udfn_mom_source_evvd_rod_old

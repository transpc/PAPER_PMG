   SUBROUTINE send_receive_mt(nnbd,nnode,spt,rpt,sintf,rintf,sia,ria,nbdom,          &
              nnsend,nnrecv,nnzs,nnzr,nnz,ia,au)
...
!DEC$ENDIF
   ...
   Integer nnbd,nnode,nnz,nnsend,nnrecv,nnzs,nnzr
   Integer spt(nnbd+1),rpt(nnbd+1),sintf(spt(nnbd+1)-1),rintf(rpt(nnbd+1)-1),nbdom(nnbd)
   Integer sia(nnsend+1),ria(nnrecv+1),ia(nnode+1)
   Real*8 au(nnz)
...
! = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = !
SUBROUTINE MD_S_R_MT2(ilv,ia,au)
...
! 
   IF(nnsend.GT.nnsend_m.OR.nnrecv.GT.nnrecv_m) THEN
       WRITE(999,*)'nnsend_m is small-1',nnsend,nnrecv,nnsend_m,nnrecv_m
       STOP
   ENDIF
...
CALL send_receive_mtc2(nnbd,nnsend,nnrecv,spt,rpt,sintf,rintf,                 &
              sia,ria,nbdom,ia,au,ndom,nnsend_m,nnrecv_m)
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - !
! = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = !
   SUBROUTINE send_receive_mtc2(nnbd,nnsend,nnrecv,spt,rpt,sintf,rintf,          &
              sia,ria,nbdom,ia,au,ndom,nnsend_m,nnrecv_m)
...
! temp:
   INTEGER(4):: i,i1,i2,j,j1,j2!,nnzs,nnzr
!   REAL(8),DIMENSION(:),ALLOCATABLE:: svar,rvar
   REAL(8) svar(sia(nnsend+1)-1),rvar(ria(nnrecv+1)-1)
! 
!   nnzs = sia(nnsend+1)-1
!   nnzr = ria(nnrecv+1)-1

!
      SUBROUTINE quality_2nd_conv_imp(xn_t,diag_g_non,src_x)
!
...
!
!.....Input
      REAL(8),DIMENSION(nf_non) :: diag_g_non
      REAL(8),DIMENSION(ncell_fp) :: xn_t
!.....Output
      REAL(8),DIMENSION(ncell_fluid) :: src_x
!.....Local variables
      INTEGER :: ii,kk
      INTEGER :: i
...
!
      IF(limiter_xn.gt.0) THEN
         CALL grad_limiter(xn_t,dqdx,limiter_xn)
!
!.....MPI communication
!
      ...
      IF(ndim.eq.2) THEN
         DO i=1,len
            ...
            IF(flux_g_nf(i1).lt.0.d0)THEN
               dx1=dxfc_non_k(i,1)
               dx2=dxfc_non_k(i,2)
               dq_non(i)=(dqdx(kk,1)*dx1+dqdx(kk,2)*dx2)*diag_g_non(i)
            ELSE
               dx1=dxfc_nf(i1,1)
               dx2=dxfc_nf(i1,2)
               dq_non(i)=(dqdx(ii,1)*dx1+dqdx(ii,2)*dx2)*diag_g_non(i)
            ENDIF
         ENDDO
      ELSE
         DO i=1,len
            ...
            IF(flux_g_nf(i1).lt.0.d0)THEN
               ...
               dx2=dxfc_non_k(i,2)
               dx3=dxfc_non_k(i,3)
               dq_non(i)=(dqdx(kk,1)*dx1+dqdx(kk,2)*dx2+dqdx(kk,3)*dx3)*diag_g_non(i)
            ELSE
               dx1=dxfc_nf(i1,1)
               dx2=dxfc_nf(i1,2)
               dx3=dxfc_nf(i1,3)
               dq_non(i)=(dqdx(ii,1)*dx1+dqdx(ii,2)*dx2+dqdx(ii,3)*dx3)*diag_g_non(i)
            ENDIF
         ENDDO
      ENDIF
!
      CALL sum_nf(1,-1,         &
                  dq_non,src_x)
!       
      END SUBROUTINE quality_2nd_conv_imp

      SUBROUTINE strtro_cupid(a,s,it,err) 
! ------------------------------------------------------------------------
!  compute gas thermodynamic properties as a function of temperature  
!  and specific volume.                                                 
!  written by Won-Jae Lee  
!
      USE STM_TBL_cupid  , ONLY: np,nt,pcrit
!
      IMPLICIT NONE
!
      REAL(8) a(1),s(26) 
...
!     INCLUDE 'machds.h' 
      ...
      INTEGER :: it,ia,ib,ic,id,lpp,lqq,jpq
      REAL(8) :: fr,fr1,frc,frc2,frd,frd2,frn,vliq,bliq
      REAL(8) :: c0,c1,c2,c3,cv
      REAL(8) :: d1,d2,f1,f2
      REAL(8) :: r,r1,r2,ra,ren
      REAL(8) :: ta,tb,pa,pb,hfg1,hfg2,dpdt1,dpdt2,dpsdts,vg
      INTEGER iunp(2) 
      REAL(8) :: unp
...
! ------------------------------------------------------------------------
!                                                                       
!     CO2 Saturation Pressure Equation - NOT USED
!  KDB Form
!      DATA b /-2.403761d1,-7.062404d3,1.663861d2,3.368548d-5,0.d0/
!  IUPAC Form
!      DATA b /11.377371d0,-6.8849249d0,-9.5924263d0,13.679755d0,        &
!	          -8.6056439d0/
!      DATA crt/647.3d0/,crp/22120000.d0/ 
!      DATA crt/304.1282d0/,crp/7.3773d6/
!---------------------------------------------------------------------
!                                                                       
...
!  computations.                                                        
   ...
   12 IF(ip.ge.nstg) GOTO 44 
      ...
      s2=.false. 
!---------------------------------------------------------------------
!     CO2 Saturation Pressure vs. Temperature
!      fr=s(1)/crt 
!      fr1=1.0d0-fr 
...
!  KDB Form
!      tt=s(1)    
!      if(tt.ge.crt) tt=crt  ! set temp as critical temp
!      ps=exp(b(1)*log(tt)+b(2)/tt+b(3)+b(4)*tt**2)
!      s(10)=ps*1000.d0
!  IUPAC Form
!      tc=304.21d0 
!      pc=7.3825d+06 
...
!      prat=b(1)*ta**1.935+b(2)*ta+b(3)*ta**2+b(4)*ta**3+b(5)*ta**4   
!      ps=pc*exp(prat) 
!      s(10)=min(ps,7.3773d6)
!  CALL STRSAT
      tt=s(1)
      call strsat_cupid(a,1,tt,ps,dpsdts,err)
      s(10)=min(ps,pcrit)
!---------------------------------------------------------------------
      jpp=jp+ntg 
...
!  liquid phase.                                                        
   58 vliq=s(11) 
      bliq=s(17) 
      s3=.false. 
   60 IF(a(jpq+1).gt.s(10)) GOTO 57 
      jp=jp+1 
      jpq=jpq+1 
      IF(jpq.ge.it3p0g) GOTO 1001 
      kp2=kp2+13 
      lpp=lqq 
      lqq=lqq+nprpntg 
      GOTO 60 
   57 s1=.false. 
      IF(a(jpq).ge.s(10)) GOTO 61 
      pa=s(10) 
      s2=.false. 
      GOTO 62 
   61 pa=a(jpq) 
   ...
   64 d1=a(lpp+2)*a(lpp)*(frn-frc2) 
      ...
      s(17)=(c1+frc*(2.d0*c2+3.d0*frc*c3))/((frn-frc2)*s(11)) 
   62 IF(s1) GOTO 65 
      IF(jp.ge.nspg) GOTO 67 
   ...
   66 IF(s3) GOTO 68 
      ...
      s(18)=(c1+frd*(2.d0*c2+3.d0*frd*c3))/((frn-frd2)*s(12)) 
      IF(s(12).le.s(3)) GOTO 68 
      s2=.true. 
      ic=id 
      jp=jp+1 
      jpq=jpq+1 
      IF(jpq.ge.it3p0g) GOTO 82 
      kp2=kp2+13 
      lpp=lqq 
      lqq=lqq+nprpntg 
      pa=a(jpq) 
   ...
   56 s(14)=a(lqq+1)+(a(id+1)-a(lqq+1))*frd 
      ...
      c0=pa 
      c1=d1 
      c2=3.d0*(a(jpq+1)-pa)-d2-2.d0*d1 
      c3=d2+d1-2.d0*(a(jpq+1)-pa) 
...
!  vapor phase.                                                         
   ...
   76 c0=a(ia) 
      ...
      c0=a(jpq) 
      c1=d1 
      c2=3.d0*(frc-a(jpq))-d2-2.d0*d1 
      c3=d2+d1-2.d0*(frc-a(jpq)) 
      s(2)=c0+ra*(c1+ra*(c2+ra*c3)) 
      s(4)=s(13)+(s(14)-s(13))*fr 
      s(5)=s(4)+s(2)*s(3) 
...
!  vapor phase, pressure less than lowest table pressure.               
   90 IF(s(1).lt.a(it4bpg+13)) GOTO 92 
      lpp=lpp+nprpntg 
   ...
   91 fr=(s(1)-a(ia))/(a(ip+1)-a(ia)) 
      s(2)=(fr*a(lpp+6)/a(ip+1)+(1.0d0-fr)*a(lqq)/a(ia))*a(jpq+1)*s(1)/ &
      s(3)                                                              
   ...
   95 frd=s(1)/a(ntg) 
      ...
      GOTO 20 
!  pressure greater than highest table pressure.                        
   81 spfl=.true. 
      GOTO 83 
   82 spfl=.true. 
      GOTO 56 
   84 s(8)=s(22) 
      ...
      s(2)=a(it3p0g)+s(4) 
      s(4)=s(14)+(a(it3p0g)*s(7)-s(1)*s(6))*s(12)*s(4) 
      s(5)=s(4)+s(2)*s(3) 
      GOTO 20 
   85 s(8)=a(lqq+4) 
      s(7)=a(lqq+3) 
      ...
      s(2)=(a(it3p0g)+s(4))*s(1)/a(ntg) 
      s(5)=s(14)+((a(it3p0g)*s(7)-s(1)*s(6))*s(4)+a(it3p0g))*a(lqq)+s(8)*frn 
      s(4)=s(5)-s(2)*s(3) 
      GOTO 20 
 1001 err=.true. 
      RETURN 
      END SUBROUTINE strtro_cupid                         

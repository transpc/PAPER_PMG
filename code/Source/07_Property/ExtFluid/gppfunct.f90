      FUNCTION gppfunct_cupid(deltapres,deltatemp,tables,ptr) 
!define win32dvf                                                        
!define erf                                                             
!define fourbyt                                                         
!define hconden                                                         
...
! 20   continue                                                         
      gppfunct_cupid=2.0d0 * 1.0d0*deltapres(1)*(tables(ptr+13)*deltatemp(1)+ &
      tables(ptr+14)*deltatemp(2)+tables(ptr+15)*deltatemp(3)+tables(   &
      ptr+16)*deltatemp(4)+tables(ptr+17)*deltatemp(5)+tables(ptr+18)*  &
      deltatemp(6))+3.0d0 * 2.0d0*deltapres(2)*(tables(ptr+19)*         &
      deltatemp(1)+tables(ptr+20)*deltatemp(2)+tables(ptr+21)*deltatemp(&
      3)+tables(ptr+22)*deltatemp(4)+tables(ptr+23)*deltatemp(5)+tables(&
      ptr+24)*deltatemp(6))                                             
      gppfunct_cupid=gppfunct_cupid+4.0d0 * 3.0d0*deltapres(3)*(tables(ptr+25)*     &
      deltatemp(1)+tables(ptr+26)*deltatemp(2)+tables(ptr+27)*deltatemp(&
      3)+tables(ptr+28)*deltatemp(4)+tables(ptr+29)*deltatemp(5)+tables(&
      ptr+30)*deltatemp(6))+5.0d0 * 4.0d0*deltapres(4)*(tables(ptr+31)* &
      deltatemp(1)+tables(ptr+32)*deltatemp(2)+tables(ptr+33)*deltatemp(&
      3)+tables(ptr+34)*deltatemp(4)+tables(ptr+35)*deltatemp(5)+tables(&
      ptr+36)*deltatemp(6))                                             
!                                                                       
      RETURN 
      END FUNCTION gppfunct_cupid                         

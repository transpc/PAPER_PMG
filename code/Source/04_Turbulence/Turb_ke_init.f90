...
!
      ...
      eddy_scale=0.07d0*turb_length_scale
!
      DO i=1,ncell_fluid
         vmean=0.0d0
!         
...
!         
         ...
         IF(iVisRatio.eq.0)THEN
            dp(i)=cmu**0.75d0*ke(i)**1.5d0/eddy_scale
         ELSEIF(iVisRatio.eq.1)THEN
            IF(turb_phase.eq.1)THEN         !gas only
               dp(i)=cell%rhog(i)*cmu*ke(i)**2/(cell%lviscosg(i)*vis_ratio)
            ELSEIF(turb_phase.eq.2)THEN     !liq only
               dp(i)=cell%rhol(i)*cmu*ke(i)**2/(cell%lviscosl(i)*vis_ratio)
            ENDIF
         ENDIF
!         
      ENDDO
